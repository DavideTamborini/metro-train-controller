...
extern volatile int emergencyBrokenLever;
extern volatile int TrafficMessageSignal;
extern volatile int StopTrainSignal;
extern volatile int EmergencyBrakingSignal;
extern volatile int MovingTrainSignal;
extern volatile int SimulatedEvent;

__task void os_idle_demon (void) {
  /* The idle demon is a system task, running when no other task is ready */
  /* to run. The 'os_xxx' function calls are not allowed from this task.  */
	for (;;) {
		emergencyBrokenLever = 0;
    TrafficMessageSignal = 0;
    StopTrainSignal = 0;
    EmergencyBrakingSignal = 0;
    MovingTrainSignal = 0;
		SimulatedEvent = 0;
    IDLE = 1;
